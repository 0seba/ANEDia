...
class Dia:
    def __init__(
        ...
        self.model: DiaModel = torch_model
        self.mlmodel: ct.models.MLModel = mlmodel
        self.dac_model = None
    ...
    def from_pretrained(
        ...
        if load_torch_model:
            try:
                torch_model = DiaModel.from_pretrained(model_name, compute_dtype=compute_dtype.to_dtype())
            except Exception as e:
                raise RuntimeError(f"Error loading model from Hugging Face Hub ({model_name})") from e
            
        # check if model is compiled else compile
        if mlmodel_path.endswith(".mlmodelc"):
            mlmodel = ct.models.CompiledMLModel(mlmodel_path, compute_units=ct.ComputeUnit.CPU_AND_NE)
        else:
            mlmodel = ct.models.MLModel(mlmodel_path, compute_units=ct.ComputeUnit.CPU_AND_NE)

        config = torch_model.config  # Get config from the loaded model
        dia = cls(mlmodel, config, torch_model, compute_dtype, device, load_dac)
