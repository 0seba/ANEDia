...
class Dia:
    def __init__(
        ...
        decoder_mlmodel_chunks: list[ct.models.MLModel],
        cross_attn_cache_mlmodel: ct.models.MLModel,
        read_state_mlmodel: ct.models.MLModel,
        config: DiaConfig,
        torch_model: DiaModel | None = None,
        ...
        self.model: DiaModel | None = torch_model
        self.encoder_mlmodel: ct.models.MLModel = encoder_mlmodel
        self.decoder_mlmodel_chunks: List[ct.models.MLModel] = decoder_mlmodel_chunks
        self.cross_attn_cache_mlmodel: ct.models.MLModel = cross_attn_cache_mlmodel
        self.read_state_mlmodel: ct.models.MLModel = read_state_mlmodel
        self.dac_model = None
    ...
    def from_local(
        ...
        try:
            state_dict = torch.load(checkpoint_path, map_location=dia.device, mmap=True)
    ...
    def from_pretrained(
        cls,
        encoder_mlmodel_path: str,
        decoder_mlmodel_path: str,
        cross_attn_cache_mlmodel_path: str,
        read_state_mlmodel_path: str,
        compute_dtype: str | ComputeDtype = ComputeDtype.FLOAT32,
        device: torch.device | None = None,
        load_dac: bool = True,
        load_torch_model: bool = False,
        ...
        if load_torch_model:
            ...
            try:
                torch_model = DiaModel.from_pretrained(
        ...
        # check if model is compiled else compile
        print("Loading mlmodel...")
        if encoder_mlmodel_path.endswith(".mlmodelc"):
            encoder_mlmodel = ct.models.CompiledMLModel(
                encoder_mlmodel_path, compute_units=ct.ComputeUnit.CPU_AND_NE
            )
        else:
            encoder_mlmodel = ct.models.MLModel(
                encoder_mlmodel_path, compute_units=ct.ComputeUnit.CPU_AND_NE
            )
        decoder_mlmlodel_chunks = []
        for path in decoder_mlmodel_path:
            if path.endswith(".mlmodelc"):
                decoder_mlmlodel_chunks.append(
        ...
        if cross_attn_cache_mlmodel_path.endswith(".mlmodelc"):
            cross_attn_cache_mlmodel = ct.models.CompiledMLModel(
                cross_attn_cache_mlmodel_path, compute_units=ct.ComputeUnit.CPU_AND_NE
            )
        else:
            cross_attn_cache_mlmodel = ct.models.MLModel(
                cross_attn_cache_mlmodel_path, compute_units=ct.ComputeUnit.CPU_AND_NE
            )
        if read_state_mlmodel_path.endswith(".mlmodelc"):
            read_state_mlmodel = ct.models.CompiledMLModel(
                read_state_mlmodel_path, compute_units=ct.ComputeUnit.CPU_AND_NE
            )
        else:
            read_state_mlmodel = ct.models.MLModel(
                read_state_mlmodel_path, compute_units=ct.ComputeUnit.CPU_AND_NE
        ...
        config = torch_model.config  # Get config from the loaded model
        dia = cls(
            encoder_mlmodel,
            decoder_mlmlodel_chunks,
            cross_attn_cache_mlmodel,
            read_state_mlmodel,
            config,
            torch_model,
            compute_dtype,
    ...
    async def async_ml_predict(self, enc_input_np):
        """Run CoreML prediction asynchronously."""
        enc_out = self.encoder_mlmodel.predict({"x_ids": enc_input_np})["enc_out"]
        state = self.cross_attn_cache_mlmodel.make_state()
        return enc_out, self.cross_attn_cache_mlmodel.predict({"enc_out": enc_out}, state=state), state

        # try:
        #     print("INFO: Running CoreML prediction asynchronously")
        #     return self.mlmodel.predict({"x_ids": enc_input_np})
        # except Exception as e:
        #     print(f"ERROR: Exception in CoreML prediction: {type(e).__name__} - {e}")
        #     traceback.print_exc()
        #     raise
    ...
    def _prepare_generation(
        ...
        enc_input_for_torch = stacked_inputs.view(
            2 * batch_size, -1
        )  # Renamed to avoid clash
        enc_input_np_for_ml = (
            enc_input_for_torch.cpu().numpy().astype(np.int32)
        )  # Renamed
        ...
        try:
            # Start CoreML prediction asynchronously
            ml_future = loop.create_task(self.async_ml_predict(enc_input_np_for_ml))

            # Run PyTorch operations synchronously while CoreML is running
            encoder_out = None
            if self.model is not None:
                enc_state = EncoderInferenceState.new(self.config, enc_input_cond)
                encoder_out = self.model.encoder(enc_input_for_torch, enc_state)

            # Await the CoreML result
            ml_output = loop.run_until_complete(ml_future)
    ...
    def generate(
        ...
        audio_prompt: (
            list[str | torch.Tensor | None] | str | torch.Tensor | None
        ) = None,
        audio_prompt_path: (
            list[str | torch.Tensor | None] | str | torch.Tensor | None
        ) = None,
