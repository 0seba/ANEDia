...
def ane_linear(
    x: Tensor, w: Tensor, bias: Optional[Tensor] = None, w_num_unsqueezes: int = 2
):
...
def simple_attention(
    ...
    k = key.split(1, 1)
    v = value.split(1, 1)
    # chunk conversion to mil not working
    ...
    for i in range(num_q_heads):
        ...
        scores = torch.matmul(k[groupi], qi)  # (1, 1, CACHE_LEN, SEQLEN)
